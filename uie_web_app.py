...
def main():
    selected_box = st.sidebar.selectbox(
        'Select from dropdown', ('Underwater Image Enhancer', 'About the App'))
...
def image_enhancer():
    ...
    file = st.file_uploader(
        "Please upload a RGB underwater image file", type=["jpg", "png"])
    if file is None:
        ...
        if image.mode != 'RGB':
            ...
            if(st.button("Enhance Uploaded Image")):
                pcafused, averagefused = underwater_image_enhancement(
                    image, flag)
...
def underwater_image_enhancement(image, flag):
    ...
    st.text(
        "Performing Average Based Fusion of Sharped Image & Contrast Enhanced Image...")
